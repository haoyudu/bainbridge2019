...
Each trial follows the structure of the original experiment. The drawing is displayed centered at the top of the screen, with three scene photographs presented in a horizontal row below. Following the original implementation, the spatial positions (left, center, right) of the three photographs are randomized on each trial using a shuffle algorithm. Participants select their response by clicking on one of the three photographs. 

```{r, out.width="60%", fig.cap="Example Trial", fig.align="center"}
knitr::include_graphics("output/results/experiment_example.png")
```
