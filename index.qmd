...
Participant exclusions were applied as preregistered, with no participants excluded for attention check failures, incomplete sessions, session timeouts, fast response times, low accuracy, or technical difficulties. Trial-level exclusions removed trials with response times below $1200$ ms. After applying exclusion criteria, $`r nrow(drawing_accuracy_adequate)`$ drawings received adequate ratings ($\geq 10$ raters per drawing), with $`r desc_stats_adequate$n_drawings[desc_stats_adequate$condition == "delayed_recall"]`$ Delayed Recall drawings and $`r desc_stats_adequate$n_drawings[desc_stats_adequate$condition == "category"]`$ Category Drawings.

The final dataset contains $`r nrow(drawing_accuracy_adequate)`$ drawings with the following variables: drawing filename, condition (Delayed Recall or Category), category, memorability (high or low), number of raters, number correct, and accuracy (proportion correct). Delayed Recall drawings achieved a mean accuracy of $`r round(desc_stats_adequate$mean_accuracy[desc_stats_adequate$condition == "delayed_recall"], 3)`$ ($SD = `r round(desc_stats_adequate$sd_accuracy[desc_stats_adequate$condition == "delayed_recall"], 3)`$), while Category Drawings achieved $`r round(desc_stats_adequate$mean_accuracy[desc_stats_adequate$condition == "category"], 3)`$ ($SD = `r round(desc_stats_adequate$sd_accuracy[desc_stats_adequate$condition == "category"], 3)`$).
...
plot_data <- drawing_accuracy_adequate %>%
  mutate(Condition = ifelse(condition == "delayed_recall", "Delayed Recall", "Category"))
...
ggplot(summary_stats, aes(x = Condition, y = mean_acc, fill = Condition)) +
  geom_bar(stat = "identity", alpha = 0.8, width = 0.4) +
  geom_errorbar(aes(ymin = mean_acc - se_acc, ymax = mean_acc + se_acc), 
                width = 0.2) +
  #geom_text(aes(y = mean_acc + se_acc, label = paste0(round(mean_acc * 100, 1), "%")), vjust = -0.3) +
  scale_y_continuous(limits = c(0, 1), labels = percent_format()) +
  scale_fill_viridis_d(begin = 0.2, end = 0.8) +
  labs(title = "Drawing-to-Picture Matching Accuracy",
       y = "Proportion Correct", x = "Drawing Condition") +
  theme_minimal() +
...
# rank-biserial correlation using effectsize (this works correctly)
...
# Cohen's d - use manual calculation to get correct sign
...
The primary hypothesis was tested using a two-tailed Wilcoxon rank-sum test comparing accuracy distributions between Delayed Recall and Category drawings. Delayed Recall drawings ($M = `r round(median(delayed_accuracy), 3)`$) were significantly more recognizable than Category Drawings ($M = `r round(median(category_accuracy), 3)`$), $W = `r wilcox_result$statistic`$, $p = `r format.pval(wilcox_result$p.value, digits = 3)`$, rank-biserial $r = `r round(rank_biserial, 3)`$.

**Assumption checks:** Both conditions violated normality assumptions (Delayed Recall: $W = `r round(shapiro_delayed$statistic, 3)`$, $p = `r format.pval(shapiro_delayed$p.value, digits = 3)`$; Category: $W = `r round(shapiro_category$statistic, 3)`$, $p = `r format.pval(shapiro_category$p.value, digits = 3)`$) but satisfied homogeneity of variance ($F = `r round(levene_result$'F value'[1], 2)`$, $p = `r format.pval(levene_result$'Pr(>F)'[1], digits = 3)`$). Following our preregistered decision rule, the Wilcoxon test was prioritized as the primary analysis given the normality violations. Welch's correction was applied for the sensitivity analysis as a conservative approach.

**Sensitivity analysis:** Delayed Recall drawings ($M = `r round(mean(delayed_accuracy), 3)`$, $SD = `r round(sd(delayed_accuracy), 3)`$) were significantly more recognizable than Category Drawings ($M = `r round(mean(category_accuracy), 3)`$, $SD = `r round(sd(category_accuracy), 3)`$), $t(`r round(t_result$parameter, 1)`) = `r round(t_result$statistic, 2)`$, $p = `r format.pval(t_result$p.value, digits = 3)`$, Cohen's $d = `r round(cohens_d, 2)`$.

**Replication success:** The preregistered criteria for successful replication were met: the Wilcoxon test showed a significant difference ($p < 0.05$) and the effect size was large ($r = `r round(rank_biserial, 3)` > 0.3$ threshold). The effect size matches the original study's very large effect.
...
# Extract by condition (same as confirmatory)
...
**Analysis including drawings with fewer than 10 raters:** When including all `r nrow(drawing_accuracy)` drawings regardless of sample size, the pattern remained consistent. Delayed Recall drawings ($M = `r round(median(delayed_full), 3)`$) were significantly more recognizable than Category Drawings ($M = `r round(median(category_full), 3)`$), $W = `r wilcox_full$statistic`$, $p = `r format.pval(wilcox_full$p.value, digits = 3)`$, rank-biserial $r = `r round(rank_biserial_full, 3)`$. This demonstrates robustness of findings across different inclusion criteria.
